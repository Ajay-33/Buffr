  ...
  return (
    <div id="profile-view-container" className="p-3 sm:p-6 max-w-4xl mx-auto space-y-4 sm:space-y-6 pb-28">
      ...
      <div className="p-3.5 sm:p-5 bg-[#11092a] border-2 border-cyan-500/80 shadow-[3px_3px_0px_#05020a] space-y-3">
        ...
        <p className="text-xs text-slate-300 font-retro">
          {currentUser
            ? `Logged in as ${currentUser.displayName || currentUser.email || 'Google Player'}. Your habits, XP, streaks, and reflections sync to Firebase Firestore in real-time across all your phones and computers.`
            : 'Sign in with your Google account to automatically sync your habits, XP, streaks, and reflections to the cloud.'}
        </p>
          ...
          {currentUser ? (
            <>
              ...
              >
                ...
                <span>SIGN OUT ({currentUser.email?.split('@')[0] || 'GOOGLE'})</span>
              </button>
            </>
          ) : (
            <button
              ...
              disabled={loading}
              className="w-full py-2.5 px-3 bg-gradient-to-r from-blue-900 to-indigo-900 hover:from-blue-800 hover:to-indigo-800 border-2 border-cyan-400 text-cyan-100 text-[10px] font-arcade flex items-center justify-center space-x-2 shadow-[3px_3px_0px_#000] active:translate-y-0.5 transition-all"
            >
